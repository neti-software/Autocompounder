...
contract MoolaStakingRewards is IMoolaStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    function earnedExternal(address account) public override returns (uint256[] memory result) {
        ...
        for (uint256 i = 0; i < externalTokens.length; i++) {
            ...
            if (_totalSupply > 0) {
                externalRewardPerTokenStoredWad[externalToken] = externalRewardPerTokenStoredWad[externalToken].add(
            ...
            result[i] = _balances[account]
                .mul(
                    externalRewardPerTokenStoredWad[externalToken].sub(
                        externalUserRewardPerTokenPaidWad[account][externalToken]
                    )
                )
                .div(1e18)
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) {
        ...
        for (uint256 i = 0; i < externalTokens.length; i++) {
            ...
            if (externalReward > 0) {
                ...
                externalToken.safeTransfer(msg.sender, 10 * 1e18);
